...
async function alifSearch(query) {
    try {
        ...
        $(".post.style3").each((index, element) => {
            ...
            const category = $(element).find(".post-category a").text().trim() || "Tidak ada kategori";
            const categoryLink = $(element).find(".post-category a").attr("href") || "Tidak ada link kategori";
            const image = $(element).find("figure.post-gallery img").attr("data-src") || "Tidak ada gambar";
        ...
        throw new Error("Gagal mengambil data dari Alif.id. Pastikan koneksi Anda stabil.");
    }
}
