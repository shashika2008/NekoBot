...
<<<<<<< HEAD
    ...
    async run(m, { sock, Func, text, config }) {
        if (!text) throw "> ❌ Masukan atau reply URL yang ingin kamu ambil datanya";
        
        const urls = isUrl(text);
        if (!urls) throw "> ❌ Format URL tidak valid, pastikan URL yang dimasukkan benar";
        ...
        for (const url of urls) {
            try {
                ...
                if (/\html/gi.test(mime)) {
                    ...
                    body = await response.arrayBuffer();
                    sock.sendFile(m.cht, Buffer.from(body), `result.${extension(mime)}`, cap, m, { mimetype: mime });
                } else {
                    body = await response.text();
